...
订单号和付款金额；支付成功页出现后直接抓取手机号码并写入同一客户。推荐码、
推荐链接等补充资料继续通过**同一个专属邮箱**自动关联。
...
成功页同时监听 CTExcel 的 XHR / fetch 响应。该网站会把订单号藏在
`history.state` 而不是地址栏中；支付跳转丢失隐藏参数时，客户端会用付款前
已保存的订单号恢复上下文，并短暂读取页面正文中的手机号。页面正文没有及时
渲染也不影响订单完成判定。成功页最多额外等待 8 秒读取号码；
HTTP 错误、请求失败和页面脚本错误会写入诊断目录中的 `network.txt`。

£1 路线的手机号实际上会在订单确认接口 `getOrderConfirmList` 中提前分配。
客户端会在付款前捕获该接口的 `msisdn`，但只在确认支付成功后写入客户管理。
因此后续成功页的订单详情接口持续 Loading 时，也可使用付款前已捕获的号码
完成当前单并继续下一单。

客户端在页面脚本初始化时同时拦截 XHR 与 fetch 的原始订单响应，避免页面
跳转后 Playwright 已取不到响应正文。若第一次确认页仍未捕获 `msisdn`，
客户端会短暂等待接口补充返回；仍没有号码时照常进入支付。支付成功页面一旦
出现，本单即标记完成，手机号保持为空并交由服务器后台继续扫描邮件。
...
历史版本已写入订单号和付款金额、但尚未补全手机号的客户会留在后台继续
同步，不阻塞下一单。新版本直接从成功页回写手机号。流程中断时会保留已完成
数量；点击“重试第 N 单并继续”后从该单恢复，不会重复已经完成的数量。
...
5. `POST /api/ctexcel-client/customers/{id}/payment-checkpoint` 回写订单号、付款金额和成功页手机号
...
该接口的实际调用由后台定时同步任务完成。客户端提交支付页显示的订单号和
付款金额，并在成功页提交页面中明确显示的手机号码。
...
“收货地址”字段。单次模式在支付成功后会立即轮询订单邮件；连续模式直接
进入下一单，订单邮件由服务器后台继续同步。在客户管理详情页仍可查看
邮箱、完整收件箱并手动触发“扫描订单邮件”。
