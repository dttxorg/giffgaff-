...
3. 批量代理池：一次粘贴几十个节点，每行一个，自动去重
4. API 动态提取：每次申请开始前重新请求接口，解析 txt 或 JSON 返回

代理池默认按顺序使用：每个节点随机使用 5–8 单后切换到下一个，最小和最大
...
正式建立客户之前，客户端会先通过桥接后的本地 SOCKS5 入口完成一次
CTExcel `443` 端口的端到端握手。浏览器启动后还会由真实 Chrome / Edge 页面
读取代理出口 IP 并写入日志，便于核对代理是否实际装载。
