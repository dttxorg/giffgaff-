...
class CTExcelAutomation:
    ...
    def _capture_payment_qr(self, page: Page) -> bytes:
        ...
        while time.monotonic() < deadline:
            ...
            # Frames can be replaced while the hosted cashier refreshes its
            # placeholder.  Re-enumerate them on every poll so the QR in a
            # newly attached frame is not missed.
            for document in self._payment_documents(page):
                with contextlib.suppress(Exception):
